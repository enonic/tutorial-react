...
We now have a Factbox component, but we still need to map it to the Enonic macro definition. The solution is to implement the Macro component.
...
This component takes care of mapping each macro to it's matching React component. In our case, `FactBox` component implements the macro called `com.enonic.app.intro:factbox` in Enonic.

It will throw an error otherwise, which will cause the RichText component's `ErrorBoundary` to render an `Error placeholder`.

== Add to RichText

Finally, import and add `Macro` to the `RichText` component:
...
You may pass additional parameters to the `RichText` component by supplying a generic interface declaring them:
...
These params will be passed on to the `Image`, `Link` and `Macro` components for your convenience.
