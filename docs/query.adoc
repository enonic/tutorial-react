...
If you open `Intro project` in Content Studio you will see that it contains a number of content types and content.
...
In order to test the query below you first have to open the `Query Playground` panel from the left-hand vertical toolbar and paste the query below into the query editor.
...
Hit play button and you should see a list of persons in the response:
...
But before you hit play, you need to set a query variable this time in the Query Playground Variables panel to specify which person you want to get data about:
...
Hit play and you should see the data about the specified person:
...
After hitting play you should see the array of movies that reference the specified person:
